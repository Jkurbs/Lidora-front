...
import RegularButton from "../components/buttons/complimentaryButton";
...
class Add extends React.Component {
  ...
  handleSearch = (text) => {
    const filter = (a, f) => {
      ...
      if (keys.length == 1) {
        return a.filter((x) =>
          x[keys[0]].toLowerCase().includes(f[keys[0]].toLowerCase())
        );
      } else
        return a.filter((x) =>
          Object.values(f).every((fv) => {
      ...
      <ScrollView style={{ height: "100%" }}>
        <View style={styles.modalHeader}>
          ...
          <View style={styles.saveButton}>
            <RegularButton
              text={"Save"}
              action={this.props.addInventoryItem.bind(this, this.state.item)}
            />
          </View>
        </View>
        <MultiSelect
        ...
        >
          ...
          <View>
            <Picker
              ...
              onValueChange={(value) => {
                if (value != "0") this.state.item.unit = value;
              }}
            >
    ...
    return (
      <ScrollView style={{ height: "100%" }}>
        ...
        <ModalTextBox
          title={"Date Added"}
          subtitle={moment
            .unix(this.props.item.dateAdded)
            .format("MMM-DD-YYYY")}
        />
      </ScrollView>
    );
