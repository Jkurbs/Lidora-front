...
function SideBarItems({userData, navigation}) {
    ...
    return (
        <View style={globalStyle.backgroundPrimary}>
            <View style={{width: 200, height: '100%', backgroundColor: colors.bgTertiary}}>
            <View style={{ flexDirection: 'column', position: "absolute", zIndex: 100, top: 20, left: 20}}>
                {/* LIDORA */}
                <Text style={[{fontSize: 20, fontWeight: '700'}, {color: colors.textPrimary}]}>LIDORA</Text>
                {/* Inner buttons */}
                <View style={{marginTop: 40}}>
                    <TouchableOpacity style={{marginBottom: 24, flexDirection: 'row', alignItems: 'center'}} onPress={() =>{alert("")} }>
                        <Image style={{width: 18, height: 18}} source={require("../assets/icon/menu-50.png")} />
                        <Text style={[{marginLeft: 8, fontSize: 13, marginRight: 8}, {color: colors.textSecondary}]}>Menu</Text> 
                    </TouchableOpacity>
                    <TouchableOpacity style={{ marginBottom: 24, flexDirection: 'row', alignItems: 'center'}} onPress={() =>{alert("")} }>
                    <Image style={{width: 20, height: 20}} source={require("../assets/icon/orders-48.png")} />
                        <Text style={[{marginLeft: 8, fontSize: 13, marginRight: 8}, {color: colors.textSecondary}]}>Orders</Text> 
                    </TouchableOpacity>
                    <TouchableOpacity style={{marginBottom: 24, flexDirection: 'row', alignItems: 'center'}} onPress={() =>{alert("")} }>
                        <Image style={{width: 18, height: 18}} source={require("../assets/icon/store-64.png")} />
                        <Text style={[{marginLeft: 8, fontSize: 13, marginRight: 8}, {color: colors.textSecondary}]}>Store design</Text> 
                    </TouchableOpacity>
                    <TouchableOpacity style={{marginBottom: 24, flexDirection: 'row', alignItems: 'center'}} onPress={() =>{alert("")} }>
                        <Image style={{width: 18, height: 18}} source={require("../assets/icon/settings-48.png")} />
                        <Text style={[{marginLeft: 8, fontSize: 13, marginRight: 8}, {color: colors.textSecondary}]}>Settings</Text> 
                    </TouchableOpacity>
                </View>               
            </View>
